...
def _validate_clip_quality(
    ...
    *,
    ...
    # Gate 1: virality_score quality (60% must be >= 7.0)
    ...
    if high_score / n < 0.6:
        raise LLMQualityError(
            ...
            f"(expected >= 60%). Model returned low-quality or default scores."
        )
    ...
    # Gate 4: duration — filter out bad clips instead of rejecting entire response
    # (LLM sometimes produces 1 outlier; keep the good ones)
    valid_clips = [c for c in clips if 10.0 <= c.duration <= 120.0]
    if valid_clips:
        response.clips = valid_clips
        clips = valid_clips
        n = len(clips)
